defmodule Speakeasy.Resolve do
  ...
  """
  def call(%{state: :unresolved, context: %{speakeasy: ctx}} = res) do
  ...
  def call(%{state: :unresolved, arguments: args} = res, %{resolver: fun, user_key: user_key}) when is_function(fun, 1) do
    do_resolve(res, fun.(args))
  end

  def call(%{state: :unresolved, arguments: args, context: ctx} = res, %{resolver: fun, user_key: user_key}) when is_function(fun, 2) do
    do_resolve(res, fun.(args, ctx[user_key]))
  end

  def call(%{state: :unresolved, arguments: args, context: ctx} = res, %{resolver: fun, user_key: user_key}) when is_function(fun, 3) do
